...
bool ConnectWiFi(void) {
  ...
  if ( strcmp(EC_Config.AP_SSID, "none")==0 ||strcmp(EC_Config.AP_SSID, "")==0 ) {
...
// Проверка авторизации  
  ...
  String out = "";
  HTTP_printHeader(out,"Конроллер умного дома v 2.0",5);
   ...
   out += "<h2>Датчики почвы</h2>\n";
   out += "<table border=1><tr>\n";
   out += "<td>Сенсор</td>\n";
   out += "<td>Время</td>\n";
   out += "<td>Счетчик</td>\n";
   out += "<td>Напряжение, В</td>\n";
   out += "<td>Температура, C</td>\n";
   out += "<td>Почва</td>\n";
   out += "</tr>\n";
   ...
   for(int i=0; i<RC_DataCount; i++ ){
      out += "<tr>\n";
      sprintf(str,"<td>0x%0x</td>\n",RC_Data[i].Id);
      out += str;
      int tm = (int)RC_Data[i].Time;
      sprintf(str,"<td>%02d:%02d</td>\n",( tm/3600 )%24,( tm/60 )%60);
      out += str;
      sprintf(str,"<td>%d</td>\n",RC_Data[i].Data[0]);
      out += str;
      sprintf(str,"<td>%0d.%03d</td>\n",RC_Data[i].Data[1]/1000,RC_Data[i].Data[1]%1000);
      out += str;
      sprintf(str,"<td>%0d.%01d</td>\n",RC_Data[i].Data[2]/10,RC_Data[i].Data[2]%10);
      out += str;
      sprintf(str,"<td>%d</td>\n",RC_Data[i].Data[3]);
      out += str;      
      out += "</tr>";
...
// Формируем строку запроса  
   ...
   sprintf(str,"/save4.php?name=%s&id=%d&tm=%ld",EC_Config.ESP_NAME,RC_Data[n].Id,RC_Data[n].Time);
   out += str;
   if( RC_Data[n].DataFlag[0] ){
      sprintf(str,"&cnt=%d",RC_Data[n].Data[0]);
      out += str;      
   }
   if( RC_Data[n].DataFlag[1] ){
      sprintf(str,"&vcc=%d",RC_Data[n].Data[1]);
      out += str;      
   }
   if( RC_Data[n].DataFlag[2] ){
      sprintf(str,"&t=%d",RC_Data[n].Data[2]);
      out += str;      
   }
   if( RC_Data[n].DataFlag[3] ){
      sprintf(str,"&soil=%d",RC_Data[n].Data[3]);
      out += str;      
   }
   out += " HTTP/1.0\r\n\r\n";
   Serial.print(out);
